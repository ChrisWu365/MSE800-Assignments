'''
Activity 3: - rewrite below sample code (from activity w3-6) without using initializer & self (if it is possible?)
...
'''
class Factorial:
    def factorial(num1):  # Factorial method now works with num1
        result = 1  # Initialized before the loop to store the product.
        for i in range(1, num1 + 1):
            result *= i
        return result
    def check_Prime(num1):  # Prime check method added
    ...
    @classmethod
    def display(cls, num1):  # Display method corrected
        print("Factorial of", num1, "is", cls.factorial(num1))
        if cls.check_Prime(num1):
