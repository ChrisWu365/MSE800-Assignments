...
class DataFileFormat_Processor:
    ...
    def calc_circle_diameter_area(self):
        ...
        radius = float(input('Enter a radius to calculate the diameter and area of the circle: '))
        diameter = 2 * radius
        area = math.pi * radius ** 2
        print(f'Given radius [ {radius} ], the diameter is: [ {diameter} ] and the area of the circle is [ {area} ]')
    ...
    def calc_symmetric_difference(self, set_a, set_b):
        ...
        print(f'The symmetric difference between {set_a} and {set_b} is: [{set_c}]')
        return set_c
